  ...
  override def toProto(te : p4rtype.TableEntry[TableMatchFields, TableAction, ActionParams, _, _]) : TableEntry =
    ...
    val matchFields =
      (te.table, te.matches) match
        ...
        case ("Process.ipv4_table", ((_, t0), t1)) => Seq(p4rtype.matchFieldToProto(1, t0.asInstanceOf[Exact])) ++ t1.asInstanceOf[Option[("hdr.ipv4.dstAddr", LPM)]].map((_, t) => p4rtype.matchFieldToProto(2, t)).toSeq
